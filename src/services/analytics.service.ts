...
export class AnalyticsService {
    ...
    try {
      ...
      await prisma.activityLog.create({
        data: {
          ...
          entityType: this.extractEntityType(event.eventData),
          entityId: this.extractEntityId(event.eventData),
  ...
  async getCourseAnalytics(
    ...
    const [views, enrollments, completions, watchTimeData, course] = await Promise.all([
      // Total views
    ...
    const averageWatchTime = watchTimeData.length > 0 ? totalWatchTime / watchTimeData.length : 0;
  ...
  async getUserBehavior(userId: string): Promise<UserBehavior> {
    const [coursesViewed, enrollments, watchTimeStr, activityLogs] = await Promise.all([
      // Courses viewed
      prisma.activityLog.findMany({
        where: {
          userId,
    ...
    for (const log of activityLogs) {
      if (log.action === 'course_view' && log.entityId) {
    ...
    return {
      userId,
      coursesViewed: coursesViewed.map((log) => log.entityId!).filter(Boolean),
      coursesEnrolled: enrollments.map((e) => e.courseId),
      totalWatchTime,
      favoriteCategories,
      averageSessionDuration,
      lastActive: activityLogs[0]?.createdAt || new Date(),
    };
  }
   ...
   */
  private extractEntityType(eventData: Record<string, unknown>): string | undefined {
   ...
   */
  private extractEntityId(eventData: Record<string, unknown>): string | undefined {
    return (eventData.courseId || eventData.materialId || eventData.videoId) as string | undefined;
  }

  /**
   * Group activity logs into sessions
   */
  private groupIntoSessions(logs: Array<{ createdAt: Date }>): Array<{ duration: number }> {
    if (logs.length === 0) return [];

    const sessions: Array<{ duration: number }> = [];
    const sessionGap = 30 * 60 * 1000; // 30 minutes

    let sessionStart = logs[0].createdAt.getTime();
    let lastActivity = sessionStart;

    for (let i = 1; i < logs.length; i++) {
      const currentTime = logs[i].createdAt.getTime();
