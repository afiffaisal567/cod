...
export class UploadService {
  ...
  async uploadVideo(file: Express.Multer.File): Promise<UploadResult> {
    ...
    if (!validateFileSize(file.size, storageConfig.limits.maxVideoSize)) {
      throw new AppError(
        `Video file too large. Max size: ${formatFileSize(storageConfig.limits.maxVideoSize)}`,
  ...
  async uploadImage(
    ...
    if (!validateFileSize(file.size, storageConfig.limits.maxImageSize)) {
      throw new AppError(
        `Image file too large. Max size: ${formatFileSize(storageConfig.limits.maxImageSize)}`,
        HTTP_STATUS.BAD_REQUEST
      );
    }
    ...
    if (options?.generateThumbnails) {
      result.thumbnails = await this.generateImageThumbnails(file.buffer, filename);
    }
  ...
  private async generateImageThumbnails(
    ...
    for (const [sizeName, dimensions] of Object.entries(sizes)) {
      ...
      const thumbnailPath = path.join('images', 'thumbnails', thumbnailFilename);
   ...
   */
  async uploadProfilePicture(file: Express.Multer.File): Promise<ProfilePictureResult> {
