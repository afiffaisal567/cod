...
async function getHandler(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return errorResponse('Failed to get comment', HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
}
...
async function putHandler(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const comment = await commentService.updateComment(id, user.userId, user.role, validation.data);
