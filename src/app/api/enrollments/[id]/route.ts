...
async function handler(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const enrollment = await enrollmentService.getEnrollmentById(enrollmentId, user.userId);
    ...
    return errorResponse('Failed to get enrollment', HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
}
