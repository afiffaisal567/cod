...
async function handler(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Archive course
    const course = await courseService.archiveCourse(id, user.userId, user.role);
    ...
    return errorResponse('Failed to archive course', HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
}
