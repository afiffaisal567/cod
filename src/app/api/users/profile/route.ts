...
async function getHandler(request: NextRequest) {
  try {
    ...
    return errorResponse('Failed to get profile', HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
}
...
async function putHandler(request: NextRequest) {
  try {
    ...
    // Update profile
    const updatedProfile = await userService.updateUser(user.userId, validation.data);
