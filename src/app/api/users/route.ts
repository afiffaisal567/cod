...
async function getHandler(request: NextRequest) {
  try {
    ...
    return paginatedResponse(result.data, result.meta, 'Users retrieved successfully');
  } catch (error) {
    if (error instanceof Error) {
      return errorResponse(error.message, HTTP_STATUS.BAD_REQUEST);
    }
    return errorResponse('Failed to get users', HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
}
...
async function postHandler(request: NextRequest) {
  try {
    ...
    return successResponse(newUser, 'User created successfully', HTTP_STATUS.CREATED);
  } catch (error) {
    if (error instanceof Error) {
      return errorResponse(error.message, HTTP_STATUS.BAD_REQUEST);
    }
